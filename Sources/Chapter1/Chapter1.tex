...
indichiamo con $ \underline{n} \defeq \{1, 2, \ldots, n\}  $.
...
\begin{remark}
	L'insieme dei tensori $ \K^{\msiz!} $ forma un $ \K $-spazio vettoriale, dotato del prodotto
	scalare euclideo, con le operazioni
	\begin{itemize}
		\item Addizione: $(T_1 + T_2)(\miwc!) = T_1(\miwc!) + T_2(\miwc!)$.
		\item Moltiplicazione per scalare: Per ogni $ \alpha \in \K, \alpha (T(\miwc!)) = (\alpha
			      T)(\miwc!)$.
...
\emph{Il problema inverso del rango:} Dato un intero positivo $ r $, nel caso matriciale è, in
...
$ invece che $ O(n_1\cdots n_d) $.

Nel generalizzare questa decomposizione, nota come \emph{Canonical Polyadic Decomposition (CP)} incontreremo alcuni
...
\end{remark}
\begin{remark}
	L'espressione sopra \todo{ref} si può anche scrivere come
	\begin{equation*}
		\| T - \sum_{r = 1}^R a_{r} \otimes b_{r} \otimes c_{r} \|^2 = \sum_{i,j,k}\abs{t_{ijk} -
			\sum_{r=1}^R a_{ir}b_{jr}c_{kr}}^2.
	\end{equation*}
\end{remark}
