...
\label{chp:tensors_intro}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{chapquote}{Italo Calvino, \textit{Palomar}}
	\ldots è ``il prato'' ciò che noi vediamo
	oppure vediamo un'erba più un'erba più un'erba…? Quello che noi diciamo ``vedere il
	prato'' è solo un effetto dei nostri sensi approssimativi e grossolani; un insieme esiste
	solo in quanto formato da elementi distinti. Non è il caso di contarli, il numero non
	importa; quel che importa è afferrare in un solo colpo d'occhio le singole pianticelle
	una per una, nelle loro particolarità e differenze. E non solamente vederle: pensarle.
	Invece di pensare ``prato'', pensare quel gambo con due foglie di trifoglio, quella
	foglia lanceolata un po' ingobbita, quel corimbo sottile\ldots \end{chapquote}

\section{Tensori}

In questo elaborato supponiamo per semplicità che in $ \K \in \{\R, \C\}  $. Denotiamo gli spazi vettoriali, che
...
gruppo delle matrici $ n \times n $ invertibili sul campo $ \K $. Dato un intero positivo $ n $,
indichiamo l'insieme dei numeri $ \{1, 2, \ldots, n\}  $ con  $ [n] $.


\begin{definition}[Mappe bilineari]
	Siano $ U,V,W $ spazi vettoriali di dimensione finita su un campo $ \K $. Una \emph{mappa
		bilineare} è un'applicazione $ \varphi : U \times V \to
		W$ lineare in ogni componente, in altre parole tale che $ \varphi(u, \cdot) $ e $ \varphi(\cdot,v) $ sono
...
dimensione finita, definiamo le forme $ n $-multilineari come applicazioni $ \varphi : A_1
	\times \cdots \times A_{n} \to \K $ lineari in ognuna delle $ n $ componenti. Data una forma $ n
$-multilineare $ \varphi : A_1 \times A_2 \times \cdots \times A_{n} $ è possibile costruire una
mappa $ n-1 $ lineare $ A_1 \times A_2 \times \cdots \times  A_{n-1} \to A_{n} $, e viceversa.
...
\begin{remark}[Spazio standard]
	...
	generale $ \K^{n_1}\otimes \K^{n_2} \otimes \cdots \otimes \K^{n_{d}} \simeq \K^{n_1 \times n_2 \times
			\cdots \times n_{d}}$) possiamo dare una definizione di tensore operativamente utile in molti contesti del
	calcolo tensoriale, dove spesso è più conveniente lavorare nello spazio standard $ \K^{n_1 \times
			\cdots \times n_{d}} $.
...
\end{definition}
\begin{warning}
	Con leggero abuso chiameremo tensore anche l'oggetto sopra scrivendo
	semplicemente $ T \in \K^{n_1 \times n_2 \times \cdots \times n_{d}} $.
\end{warning}
\begin{remark}
	Dato $ T \in A \otimes B \otimes C $, possiamo sempre passare da un tensore nello spazio $ A
		\otimes B \otimes C $ a un tensore in $ \K^{\bm{a} \times \bm{b} \times \bm{c}} $ nel modo seguente. Fissate
	...
	scrivere $ T = \sum_{ijk} T_{ijk} e_{i} \otimes g_{j} \otimes h_{k} $. Al variare degli indici, $
		T_{ijk} $ descrive un array tridimensionale (``a forma di parallelepipedo'') in $ \K^{\bm{a}
			\times \bm{b} \times \bm{c}} $, che chiamiamo ancora $ T $. Questo ragionamento si può
	estendere per i tensori di ordine $ d $ generico (anche per le matrici).
\end{remark}
\begin{remark}
...
\begin{example}[Tensore della matrix multiplication~\cite{landsberg2015geometry}]
	Come anticipazione di quanto approfondiremo nel Capitolo~\ref{chp:matmul}, la moltiplicazione tra
	matrici di taglie $ m \times n
	$ e $ n \times p $ si può vedere come mappa
	bilineare $ \K^{m \times n} \times \K^{n \times p} \to \K^{m \times p} $. Tale mappa è
	rappresentabile tramite
	un tensore tridimensionale nello spazio $(\K^{m\times n})^* \otimes (\K^{n\times p})^* \otimes
		\K^{m \times p} $. Indicando con $ E_{rc} $ la base canonica dello spazio delle matrici, il \emph{tensore della matrix
		multiplication} ha coordinate date da
	\begin{equation*}
		T_{ijk} = \begin{cases}
			1 & \text{se } E_{ij}E_{jk} =E _{ik}, \\
			0 & \text{ altrimenti.}
		\end{cases}
	\end{equation*}
	Di conseguenza, il tensore può essere scritto nella forma
	\begin{equation*}
		T = \sum_{i=1}^m \sum_{j=1}^n \sum_{k = 1}^p E_{ij}^* \otimes E_{jk}^* \otimes E_{ik}.
	\end{equation*}
\end{example}

\begin{example}
	Un esempio di tensori sono gli array di dati multidimensionali che codificano più caratteristiche.
	Ad esempio un video di $ 600 $ frames e risoluzione $ 1920 \times 1080 $ pixels può essere visto
	come un tensore a tre indici di formato $ 600 \times 1920 \times 1080 $ in cui ogni entrata contiene l'intensità di ogni pixel ad ogni
	istante di tempo. Esploreremo questo tipo di tensori nel Capitolo~\ref{chp:cp_applications}.
\end{example}

\begin{remark}
	L'insieme dei tensori $ \K^{\msiz!} $ forma un $ \K $-spazio vettoriale con le operazioni di addizione e moltiplicazione per scalare
	\begin{equation*}
		\begin{aligned}
			(T_1 + T_2)(\miwc!) & = T_1(\miwc!) + T_2(\miwc!)                           \\
			\alpha (T(\miwc!))  & = (\alpha T)(\miwc!), \text{ per ogni } \alpha \in \K
		\end{aligned}
	\end{equation*}
	Possiamo dotare questo spazio di un prodotto scalare $ \angb{T_1, T_2} = \Vec(T_1)^{\top}
		\Vec(T_2)$, che induce una norma $\| \Vec(T)\| = \sqrt{\sum_{i_1=1}^{n_1}\sum_{i_2=1}^{n_2}\cdots
			\sum_{i_{d}=1}^{n_{d}} (T_{i_1,i_2,\ldots,i_{d}})^2 }$.
	Osserviamo che $ \| \cdot \| $ è la norma Frobenius sullo spazio $ \K^{\msiz!} $, che è indotta dal prodotto scalare sopra, che
	a sua volta è indotto dal prodotto scalare euclideo di $ \K^{n_1 n_2 \cdots n_{d}} $.
\end{remark}
...
Operativamente, dati due vettori $ a = \begin{bmatrix} a_1, \ldots, a_m \end{bmatrix}^{\top} \in \K^m $ e $ b =
	\begin{bmatrix} b_1, \ldots, b_n \end{bmatrix}^{\top} \in \K^n  $, possiamo calcolare il prodotto
tensore come
\begin{equation*}
	a \topp b = ab^{\top} =
	\begin{bmatrix} a_1b_1 & a_1b_2 & \ldots & a_1b_n \\ a_2b_1 & a_2b_2 &
                \ldots & a_2b_n                   \\
                \vdots & \vdots & \ddots & \vdots \\
                a_mb_1 & a_mb_2 & \ldots & a_mb_n
	\end{bmatrix} \in \R^{m\times n}.
\end{equation*}
Questo segue dalla valutazione di $ T = a \otimes b \in \K^{m \times n}$ sulla base canonica $
	e_{i}^* $:
\begin{equation*}
	T_{i,j} = T(e_{i}^*, e_{j}^*) = (a \otimes b)(e_{i}^*, e_{j}^*) = e_{i}^*(a) e_{j}^*(b) = a_{i}b_{j}.
\end{equation*}

Analogamente, in più dimensioni, presi $ a_{1} \in \K^{n_1}, \ldots, a_{n} \in \K^{n_n} $, il loro prodotto tensore è
\begin{equation}
	\label{eq:rk1}
	T = a^{(1)} \topp a^{(2)} \topp \cdots \topp a^{(d)} \in \K^{n_1 \times n_2 \times \cdots \times
		n_{d}}
\end{equation}
tale che la sua scrittura in componenti è
\begin{equation}
	\label{eq:rk1_components}
	T_{\miwc} = a_{i_1}^{(1)}a_{i_2}^{(2)} \cdots a_{i_d}^{(d)}
\end{equation}

Questo ci permette di definire la seguente quantità fondamentale:
\begin{definition}[Rango di un tensore]
	Il rango di un tensore $ T \in A_1 \otimes A_2 \otimes \cdots \otimes A_{n}$ è il minimo intero
	positivo $ r $ tale che $ T $ si può scrivere come somma di tensori di rango uno. Lo indicheremo con $ \rk(T) = r $.
\end{definition}

\section{Ordinamenti}

In molte situazioni è utile passare da una $ d $-upla di indici a un indice lineare, e viceversa,
per vettorizzare un tensore o appiattirlo su un tensore di formato più basso (come un tensore su una
matrice, o un tensore di ordine $ 4 $ su uno di ordine $ 3 $).
...
\begin{definition}[Ordinamento naturale]
	...
	$ s_{j+1} = n_1 \cdots n_{j} = \prod_{h=1}^{j}n_h = s_jn_j$, per ogni $ j = 1, \ldots, d-1 $.
	L'inversa di $ \L $ è data dalla mappa
	\begin{equation*}
		\begin{aligned}
			\L^{-1} \colon [\prod_{j=1}^d n_j] & \to \mdom!                                                                               \\
			l                                  & \mapsto \left(1 + \left\lfloor \frac{ (l-1)\pmod{n_1s_1}}{s_1}\right\rfloor, \ldots, 1 +
			\left\lfloor \frac{ (l-1)\pmod{n_ds_d}}{s_d} \right\rfloor \right).
		\end{aligned}
	\end{equation*}
...
\begin{proof}[Buona definizione]
	...
	\begin{equation*}
		\L(\beta) = l.
	\end{equation*}
	Per comodità prendiamo $ k = l-1 $, dove $ 0 \le k
		< \prod_{j=1}^d n_{j}$. Ricordando che $ n_{j}s_{j} = s_{j+1} $, per definizione di
	linearizzazione si ha
	\begin{align}
		\L(\beta) & = \L\left(\beta_1, \ldots, \beta_{d}\right) =  \notag                  \\
		          & =1 + \sum_{j = 1}^d s_j\left(\left\lfloor \frac{k \pmod{s_{j+1}}}{s_j}
		\right\rfloor \right).\label{eq:linear_inverse_beta}
	\end{align}
	Consideriamo la scrittura in base mista $ k = \sum_{h=1}^d c_{h}s_{h} $, dove $ 0 \le c_{h} <
		n_{h} $, allora vale che $ k \pmod{s_{j+1}} = \sum_{h=1}^j c_{h}s_{h} $, da cui dividendo per $
		s_{j}$,
	\begin{equation*}
		\frac{k \pmod{s_{j+1}}}{s_{j}} = c_{j} + \sum_{h=1}^{j-1} c_{h}s_{h} = c_{j} + \frac{k
			\pmod{s_{j}}}{s_{j}}.
	\end{equation*}
	Siccome $ k \pmod{s_{j}} < s_{j} $ allora $ \left\lfloor c_{j} + \frac{k \pmod{s_{j}}}{s_{j}}
		\right\rfloor = c_{j} $, per ogni $ j= 1,\ldots,d $, e sostituendo questi passaggi nell'Equazione~\eqref{eq:linear_inverse_beta},
	\begin{equation*}
		\L(\beta) = 1 + \sum_{h=1}^d s_{h}c_{h} = 1 + k = 1 + (l-1) = l.
	\end{equation*}
\end{proof}
...
\begin{example}
	...
	\begin{equation*}
		\begin{aligned}
			 & \L^{-1}(i,j,k)                       \\
			 & = \left( \left\lfloor (l-1) \pmod{m}
			\right\rfloor, 1 + \left\lfloor \frac{(l-1) \pmod{mn}}{m} \right\rfloor, 1 + \left\lfloor
			\frac{(l-1) \pmod{mnp}}{mn} \right\rfloor \right).
		\end{aligned}
	\end{equation*}
\end{example}
...
\begin{definition}
	...
	\begin{equation*}
		\begin{aligned}
			\Vec \colon \K^{n_1 \times n_2 \times \cdots \times n_{d}} & \to \K^{\prod_{j=1}^d n_j} \\
			T                                                          & \mapsto x                  \\
...
\begin{definition}[Prodotto di Kronecker]
	\label{def:krn}
	Date due matrici $ A \in \K^{m \times n} $ e $ B \in \K^{p \times q} $, il loro prodotto
	di Kronecker è la matrice
	\[
		C = A \boxtimes B =
		\begin{bmatrix}
		...
		\end{bmatrix}  \in  \K^{mp \times nq}, \text{dove } c_{\alpha \beta} = a_{ij}b_{kl},
		.\]
...
\begin{remark}
	...
	Dalla definizione di prodotto di Kronecker, segue che
	\begin{equation*}
	...
	\begin{equation*}
		...
		\end{aligned}
	\end{equation*}
	dove la seconda uguaglianza segue dalla proprietà $ (AC \krn BD) = (A \krn B)(C \krn D) $ del
	prodotto di Kronecker. Siccome le matrici $ U,U' $ e $ V,V' $ sono ortogonali (e quindi
	...
	\end{equation*}
\end{remark}
\begin{remark}[Prodotto di Kronecker come vettorizzazione del prodotto tensore]
	\label{rmk:krn_tensor}
	Dati due vettori $ u \in \K^m $ e $ v \in \K^n $, il loro prodotto di Kronecker si può vedere come
	...
	\begin{equation}
		...
		\end{bmatrix}  \right) =
		\begin{bmatrix}[c|c|c|c] u_1v & u_2v & \ldots &  u_mv \end{bmatrix} = u \krn v.
	\end{equation}
...
Introduciamo una scrittura di un tensore in tensori elementari, utile a risolvere alcuni
problemi che affronteremo nei prossimi capitoli.

\emph{Il problema inverso del rango:} Dato un intero positivo $ r $, nel caso matriciale è semplice costruire una matrice di
...
\emph{Risparmio computazionale:} Cerchiamo una decomposizione di rango basso di un tensore $ T \in \Rmsiz! $ per ridurre il
costo di rappresentazione in memoria e delle operazioni algebriche che lo coinvolgono. Nel caso matriciale, data una
matrice $ X \in \R^{n_1\times n_2} $ di rango $ r $, possiamo scrivere
\[
	X = UV^{\top} = \begin{bmatrix}[c|c|c] u_1 & \cdots & u_r  \end{bmatrix}  \begin{bmatrix}[c|c|c]  v_1 & \cdots
                            & v_r\end{bmatrix}^{\top} =
...
$ invece che $ O(n_1\cdots n_d) $. Nel farlo incontreremo alcuni
ostacoli, che discuteremo nella \S~\ref{sec:cp_rank}. Dimostreremo inoltre alcune condizioni
necessarie per l'unicità nel Capitolo~\ref{cp:cp_uniqueness}.
...
\begin{definition}[Canonical Polyadic Decomposition (CP)]
	...
	ammette una \emph{Canonical Polyadic Decomposition} (abbreviata, \emph{decomposizione CP}) di
	rango al più $ r \in \N $ se si può scrivere come somma di $ r $ tensori di rango uno
	\begin{equation*}
...
\end{remark}
\begin{remark}
	Possiamo riformulare il rango $ r =\rk(T) $ come il minimo $ r $ tale per cui $ T $ ammette una
	fattorizzazione CP.
\end{remark}
...
\end{remark}
\begin{remark}
	Possiamo riferirci alla CP sia come decomposizione (data dalla somma dei termini di rango uno)
	oppure come fattorizzazione (data dai fattori $ U_1, \ldots, U_{d} $).
\end{remark}

\begin{remark}
	Tramite la notazione sopra, la fattorizzazione CP si può scrivere nelle seguenti formulazioni equivalenti:
	\begin{enumerate}[label=(\roman*)]
		\item $ T = \sum_{l=1}^r u_l^1 \topp \cdots \topp u_l^d$.
		\item $ \Vec(T) = \sum_{l=1}^r u_l^d \krn \cdots \krn u_l^1$.
		\item $T_{\miwc} = \sum_{l=1}^r U_1(i_1,l)U_2(i_2,l)\cdots U_d(i_d,l)$.
	\end{enumerate}
\end{remark}
...
\begin{definition}[Tensor slice]
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\textwidth, keepaspectratio]{Sources/Chapter1/figures/slices.pdf}
		\caption{Slices di un tensore di ordine 3.}
...
\begin{definition}[Mode-$k$ fiber]
	...
	\emph{mode-$k$ fiber}. Nel caso $ d=3 $ denotiamo le fibre come fibre colonna $t_{:jk}$ (mode-1), fibre riga $t_{i:k}$ (mode-2) e fibre tubo $t_{ij:}$ (mode-3), come illustrato in Figura~\ref{fig:tensor_fibers}.
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\textwidth, keepaspectratio]{Sources/Chapter1/figures/fibers.pdf}
		\caption{Fibre di un tensore di ordine 3.}
...
\begin{definition}[Mode-$k$ unfolding]
	...
	su $ [n_1] \times \cdots [n_{k-1}] \times [n_{k+1}] \times \cdots \times [n_{d}] $. In particolare si ha
	\[
		(\Tm{T}{k})_{i,j} = T_{\siwc{i}} \text{, con } i = i_k, \; j = \L(\siwc)
		.\]
\end{definition}
\begin{remark}
	Possiamo pensare all'unfolding nel modo $ k $ come a rimappare l'indice $ i_{k} $ sulle righe e
	l'indice $ (i_1, \ldots, i_{k-1}, i_{k+1}, \ldots, i_{d}) $ sulle colonne. Le
	Figure~\ref{fig:fibers_unfolding} e~\ref{fig:unfoldings} mostrano alcuni esempi di unfolding nel caso $ 3 $-dimensionale.
\end{remark}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.85\textwidth, keepaspectratio]{Sources/Chapter1/figures/fibers_unfolding.pdf}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth, keepaspectratio]{Sources/Chapter1/figures/unfoldings.pdf}
...
In alcune situazioni è utile cambiare il formato del tensore.
...
\begin{definition}[Reshape]
	...
	che $ \prod_{i=1}^{d'} m_j = \prod_{j = 1}^{d} n_j$, il tensore
	\begin{equation*}
		Y = \reshape(T, \msiz![m][d']) \in \Rmsiz![m][d']
	\end{equation*}
	è definito come
	\[
		Y_{i_1, \ldots, i_{d'}}= T_{j_1, \ldots, j_d}
		,\]
	per ogni $(i_1, \ldots, i_{d'}),(j_1, \ldots,
		j_d) $ tali che
	\begin{equation*}
		\L(i_1, \ldots, i_{d'}; \msiz![m][d']) = \L(j_1, \ldots,
		j_d; \msiz!).
	\end{equation*}
\end{definition}
\begin{remark}
...
\begin{remark}
	...
	\begin{equation*}
		\Tm{T}{1} = \begin{bmatrix}[c|c|c]
			T_{:, \ldots, :,1} & \dots & T_{:, \ldots, :, n_{d}}
		\end{bmatrix} \in \R^{n_1 \times
			\prod_{j=2}^d n_{j}}.
	\end{equation*}
\end{remark}
\begin{remark}
...
\begin{proof}
	...
	$. Per definizione di tensore di rango uno la scrittura in componenti di $ T $ è
	\begin{equation*}
...
In molte situazioni è utile moltiplicare un tensore con una matrice. La seguente definizione risolve
...
Rispetto al caso matriciale, il problema di calcolare il rango e una fattorizzazione CP di un tensore si
complicano notevolmente. Ci sono molte domande aperte riguardo al rango, ad esempio non si ha una
risposta generale a domande pienamente risolte per le matrici:
\begin{enumerate}
	...
	\item Qual è il rango di una successione di tensori al limite?
\end{enumerate}
...
tensori di un rango fissato $ r $ non è chiusa\footnote{Non ne precisiamo la topologia, per una
...
\begin{example}
	...
	\end{equation}
	che essendo combinazione di due tensori di rango uno, ha rango 2.
	...
	\begin{equation}
		\begin{aligned}
			T_\varepsilon & =  \frac{1}{\varepsilon} a_1 \topp b_1 \topp c_2 + a_1 \topp b_1 \topp c_2 + a_1 \topp b_2 \topp c_1
		...
		\end{aligned}
	\end{equation}
	Quindi $ T_\varepsilon - T \xrightarrow[\varepsilon \to 0]{}  0 $.
\end{example}
...
\end{remark}
\begin{remark}
	Nell'esempio sopra, $ \brk(T) \le 2 $.
\end{remark}
Il border rank ammette anche la seguente interpretazione geometrica:
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.80\textwidth, keepaspectratio]{Sources/Chapter1/figures/border-rank.pdf}
	\caption{Nella varietà dei tensori $ 2\times 2\times 2 $ una successione di tensori di rango $2$
		può convergere a un tensore di rango $3$ al bordo tra i tensori di rango due e quelli di rango 3.}
	\label{fig:border_rank}
\end{figure}




\subsection*{Difficoltà di calcolo}
\label{sec:computing_rank}
Il costo di calcolare una fattorizzazione CP usando un algoritmo esatto è NP-Hard~\cite{haastad1989tensor}. Non
solo; il problema di trovare un tensore approssimante di un certo rango è computazionalmente
difficile. In altre parole, la miglior approssimazione di rango $ r $ è data da dei vettori $ u_{i}
	\in \R^{n_1}, v_{i} \in \R^{n_2}, \ldots, w_{i} \in \R^{n_{d}}   $, per $ i = 1, \ldots, r $ che minimizzano
\begin{equation*}
	\| A - u_1 \otimes v_1 \otimes \cdots \otimes w_1 - \cdots - u_{r} \otimes v_{r} \otimes \cdots
	\otimes w_{r}\|
\end{equation*}
...
ritenuto infattibile~\cite{desilva2008tensorrankillposednessbest}.
...
relativo rispetto al rango $ r-1 $. Possiamo definire l'\emph{errore relativo} per un tensore $ T $
a tre indici come
\begin{equation}
	\label{eq:rel_error}
...
\emph{rango massimo} è definito come il massimo dei ranghi realizzabili per dei tensori $ T \in
	\R^{n_1 \times n_2 \times  \cdots \times n_{d}} $ di una certa taglia fissata. In altre parole,
\begin{equation*}
	\rk_{\text{max}}(\R^{n_1\times n_2 \times \cdots \times  n_{d}}) = \max \{ \rk(T) \mid T \in
	\R^{n_1 \times n_2 \times \cdots \times n_{d}}\}.
\end{equation*}
Sfortunatamente, i ranghi massimi per la maggior parte dei
tensori di ordine superiore sono sconosciuti. I casi in cui il rango massimo è noto sono elencati
nella Tabella~\ref{tab:max_ranks}. Il primo risultato nella tabella implica che un tensore di taglia
$2 \times 2 \times 2$ ha un rango massimo pari a $3$; a differenza della matrici, il rango può
superare le dimensioni dei singoli spazi.
...
È inoltre possibile derivare un limite superiore debole per il rango massimo~\cite[Pag. 275]{BK25},
\begin{equation}
...
Mentre le matrici random hanno rango massimo con probabilità uno\footnote{L'idea alla base è che, presa una matrice
	...
	sul luogo di zeri dà probabilità nulla. La tesi si ha passando al complementare.}, per un tensore di un
formato fissato potrebbe esserci più di un rango con probabilità positiva. Questo fenomeno è noto come rango tipico.

Fissiamo una misura di probabilità $ P $, e sia $ \mathcal{D}(n_1, n_2, \ldots, n_{d})$ una distribuzione di probabilità continua sullo spazio
dei tensori $ \K^{n_1 \times n_2 \times \cdots \times n_{d}} $ (un esempio sono i tensori a entrate
gaussiane).
\begin{definition}
	...
	\begin{equation*}
		\rk_{\text{typical}} = \{r \mid P \left( \rk(T) = r, \; T \sim \mathcal{D}(n_1, \ldots,
			n_{d})\right) > 0 \text{ per ogni } \mathcal{D}(n_1, \ldots,
		n_{d})\}.
	\end{equation*}
\end{definition}

Per le matrici il rango tipico è unico e coincide con il massimo rango possibile. Per i
tensori di ordine superiore a due non è così: su $\mathbb{R}$ possono coesistere
...
Selezioniamo alcuni formati e generiamo dei tensori casuali con entrate estratte
...
usando l'algoritmo ALS (che introdurremo in \S~\ref{sec:computing_cp}) su un campione di $ 200 $ tensori, facendo $ 120 $ restarts e un
...
\begin{table}[htbp]
	\centering
	\small
	\caption{Ranghi tipici noti su $\mathbb{R}$ per tensori di dimensione $m \times n \times p$ con $2 \le m \le n \le p$ (da Ballard e Kolda~\cite[Tabella~16.2]{BK25}).}
...
Nel caso matriciale ogni matrice $ m \times n $ reale ha lo stesso
rango se visto come elemento di $ \R^{m\times n} $ o di $ \C^{m \times n} $, ma per i tensori questo è
...
Data la complessità del problema del rango possiamo stimarlo con la seguente
proposizione, dalla quale intuiamo che non sarà semplice trovare stime generali più precise---queste spesso coinvolgono tecniche algebrico-combinatorie avanzate. Vedremo alcuni esempi di
	...
	\begin{equation*}
		\begin{aligned}
			...
			\cdots \krn u_h^1 \right)^{\top}                                                                   \\
			          & = U_j \begin{bmatrix}[c|c|c] u_1^d \krn \cdots \krn u_1^1 & \cdots & u_r^d \krn \cdots
...
\begin{proposition}[Teorema del rango]
	\label{th:rank}
	Sia $ T \in \Rmsiz! $ un tensore che ammette una fattorizzazione $ T = \dsqr{U_1, \ldots,
			U_d} $ di rango $ r $, e sia $ r_j = \rank(\Tm{T}{j}) $ il rango dell'unfolding $ \Tm{T}{j} $ con $ j = 1, \ldots, d $. Allora vale che
	\begin{equation}
...
\begin{proof}
	...
	$ è una matrice di taglia $ n_{j} \times r  $ di rango $ r_{j} $.
	...
	dall'Osservazione~\ref{rmk:krp_as_span} segue che $ \rank \left(U_d \krp \cdots \krp U_2 \right) =r $.
	Applicando l'Osservazione~\ref{rmk:krp_sub_krn} a più fattori troviamo che $ U_d \krp \cdots \krp U_r $ è
	sottomatrice di $ U_d \krn \cdots \krn U_2 $. Notiamo che i fattori $ U_{j} $ hanno rango $ r_{j}
	$ in quanto
	\begin{equation*}
		\begin{aligned}
			r_{j} & = \rank(\Tm{T}{j}) = \rank(U_{j}\left( U_{d} \krp U_{d-1} \krp \cdots \krp
			U_{d+1} \krp U_{j-1} \krp \cdots \krp U_{1} \right)^{\top})                        \\
			      & \le \min \{\rank(U_{j}), \rank(\left( U_{d} \krp U_{d-1} \krp \cdots \krp
			U_{d+1} \krp U_{j-1} \krp \cdots \krp U_{1} \right)^{\top})\}                      \\
			      & \le \rank(U_{j}) \le r_{j},
		\end{aligned}
	\end{equation*}
	per ogni $ j = 1, \ldots, d $.
	Siccome il rango del prodotto di Kronecker è
	moltiplicativo (Osservazione~\ref{rmk:rk_krn_prod}) troviamo
	\begin{equation*}
		r \le \rank \left(U_d \krn \cdots \krn U_2 \right) = \rank (U_{d}) \cdots \rank(U_{2}) = r_2 \cdots r_d.
	\end{equation*}
...
valutare l'errore di approssimazione dell'Equazione~\eqref{eq:rel_error} o generare numericamente un tensore. Un modo conveniente
di farlo è il seguente. Data una fattorizzazione $ T = \dsqr{A,B,C} \in \R^{m\times n\times p} $ di
rango $ r $,
poiché $ \vec(T) = \left( C \krp B \krp A \right)\bm{1}_{r} $, saremmo tentati di formare la matrice $ C
	\krp B \krp A $ e moltiplicarla per il vettore di tutti uni $ \bm{1}_{r} $, ma questa matrice ha più
entrate del risultato finale. L'approccio vincente è considerare l'unfolding $ \Tm{T}{1} = A \left( C \krp B
	\right)^{\top} $ della Proposizione~\ref{lhm:krp_unfoldings}, e studiare il sistema
\begin{equation*}
	\begin{cases}
		R = C \krp B,  \\
		Y = AR^{\top}, \\
...
Il risultato intermedio $ R \in \R^{np \times r} $ ha costo $ O(npr) $, mentre formare $ Y $ costa $
	O(mnpr) $. Essendo un unfolding sul modo uno, non paghiamo il costo di permutazioni.
...
problema non convesso (esistono molti minimi locali). Ricordiamo anche che in generale un tensore può avere fattorizzazioni CP multiple.
	...
	\begin{equation*}
		\begin{aligned}
			...
			\begin{bmatrix} a_1^{\top} \krn b_1^{\top} \\ \vdots \\
				a_{r}^{\top} \krn b_{r}^{\top}
			\end{bmatrix} \begin{bmatrix}[c|c|c]
...
essa è convessa nelle singole variabili. Ovvero, fissate $ B $ e $ C $, ottimizzare solo su $ A $
...
La soluzione può essere espressa in termini delle equazioni normali
\[
...
\end{equation}
Sia $ Z \defeq (C \krp B) = C^{\top}C \had B^{\top}B \in \R^{r\times r} $. Osserviamo che $ Z $ soddisfa
\begin{equation*}
	x^{\top}Zx = x^{\top}(C \krp B)^{\top}(C \krp B)x = ((C \krp B)x)^{\top}((C \krp B)x) = \| (C \krp
	B)x \|_{2}^2 \ge 0
\end{equation*}
per ogni $ x \in \R^r $, ossia è semidefinita positiva. In particolare dall'equazione sopra segue che quando $ C
	\krp B$ è full-rank $ Z $ è positiva definita: in tal caso possiamo risolvere con la fattorizzazione di Cholesky il sistema
\begin{equation}
	AZ = \Tm{T}{1}(C\krp B)= M,
\end{equation}
...
\begin{remark}
	Scomponiamo i costi per risolvere il sistema sopra:
	\begin{itemize}
		\item Calcolare $ \Tm{T}{1}(C \krp B) $ costa $ O(mnpr) $, in quanto calcolare $ C \krp B $
		      costa $ O(npr) $; essendo $ C \krp B $ una matrice $ np \times r $ e $ \Tm{T}{1} $ una matrice
		      $ m \times np $, il loro prodotto costa $ O(mnpr) $.
		\item Calcolare $ (C^{\top}C \had B^{\top}B) $ costa $ O(r^2n + r^2 p + r^2) $ in quanto $
			      C^{\top}C $ costa $ r^2n $, $ B^{\top}B $ costa $ r^2p $ ed essendo entrambe due matrici $ r
			      \times r $, il loro prodotto di Khatri-Rao costa $ O(r^2) $.
		\item La risoluzione con Cholesky costa $ O(r^3) $, e ricavare $ A $ costa $ O(r^2m) $.
	\end{itemize}
...
dell'Equazione~\eqref{eq:lsq_matrix}, prima al variare di $ A $, poi $ B $ e poi $ C $. Alla luce
dei calcoli sopra, risolvere i problemi dati dall'Equazione ha costo $
	O(mnpr) $, che quindi è anche il costo dell'algoritmo ALS. Riassumiamo questo schema
...
\begin{remark}
	...
	arrestiamo l'algoritmo quando sta ``stagnando'', ovvero quando l'errore relativo fra due iterazioni
