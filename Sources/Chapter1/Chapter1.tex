...
\section{Il prodotto di Kroneker}
...
\begin{remark}[Prodotto di Kroneker come vettorizzazione del prodotto tensore]
	...
	Dati due vettori $ u \in \K^m $ e $ v \in \K^n $, il loro prodotto di Kroneker si può vedere come
...
\[
	X = UV^{\top} = \begin{bmatrix}  u_1 | \cdots | u_r  \end{bmatrix}  \begin{bmatrix}  v_1 | \cdots
		| v_r\end{bmatrix}^{\top} =
...
\end{remark}
\begin{remark}
	Possiamo rifrasare il rango $ r =\rk(T) $ come il minimo $ r $ tale per cui $ T $ ammette una
	fattorizzazione CP.
\end{remark}
...
proposizione, dalla quale intuiamo che non sarà semplice trovare stime generali più precise---queste spesso coinvolgono tecniche algebrico-combinatoriche avanzate. Vedremo alcuni esempi di
...
\begin{proof}
	...
	\end{equation*}
	per ogni $ j = 1, \ldots, d $.
	Siccome il rango del prodotto di Kroneker è
...
\begin{remark}
	...
	\begin{itemize}
		\item Calcolare $ \Tm{T}{1}(C \krp B) $ costa $ O(mnpr) $, in quanto calcolare $ C \krp B $
		      costa $ O(mpr) $; essendo $ C \krp B $ una matrice $ np \times r $ e $ \Tm{T}{1} $ una matrice
		...
		\item Calcolare $ (C^{\top}C \had B^{\top}B) $ costa $ O(r^2n + r^2 p + r^2) $ in quanto $
			      ...
			      \times r $, il loro prodotto di Katri-Rao costa $ O(r^2) $.
