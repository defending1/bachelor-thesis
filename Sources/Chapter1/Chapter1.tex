...
Operativamente, dati due vettori $ u = \begin{bmatrix} u_1, \ldots, u_m \end{bmatrix}^{\top} \in \K^m $ e $ v =
...
\end{equation*}
In più dimensioni, presi $ a_{1} \in \K^{n_1}, \ldots, a_{n} \in \K^{n_n} $, il loro prodotto tensore è
\begin{equation}
	\label{eq:rk1}
...
\begin{definition}[Ordinamento naturale]
	...
	$ s_{j+1} = n_1 \cdots n_{j} = \prod_{h=1}^{j}n_h = s_kn_k$, per ogni $ j = 1, \ldots, d-1 $.
...
\begin{proof}[Buona definizione]
	...
	\begin{align}
		\L(\beta) & = \L\left(\beta_1, \ldots, \beta_{d}\right) =  \notag                                                 \\
		          & =1 + \sum_{j = 1}^d s_j\left(\left\lfloor \frac{k \pmod{s_{j+1}}}{s_j} \label{eq:linear_inverse_beta}
		\right\rfloor \right).
	\end{align}
...
\begin{definition}[Prodotto di Kroneker]
	...
	\[
		...
		\end{bmatrix}  \in  \K^{mp \times nq}, \text{dove } c_{kl} = a_{ij}b_{nl},
		.\]
