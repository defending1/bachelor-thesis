...
\begin{remark}
  Un tensore $ T \in \R^{m \times n\times 1} $ è una matrice $ m \times n $, e più in generale
  un tensore $ T \in \R^{n_1\times n_{d-1} \times 1} $ è un tensore di dimensione $ n_1\times
  n_2\times \cdots \times  n_{d-1} $.
\end{remark}

\begin{definition}
  Un \emph{tensore} $ T  $ di ordine $ d $ è un array multidimensionale con entrate definite sul campo $
  \K $, che denotiamo come $  T \in \Kmsiz{\K} $. Indicizziamo gli elementi di un tensore come
  $ T_{\miwc} $, dove $ (\miwc) \in \mdom$.
\end{definition}
...
\begin{definition}[Prodotto esterno]
  ...
  \begin{equation}
    \label{eq:rk1}
  T = v^1 \topp v^2 \topp \cdots \topp v^d \in \Rmsiz
  \end{equation}
   tale che la sua scrittura in componenti è
     \[
       T_{\miwc} = v_{i_1}^1v_{i_2}^2 \cdots v_{i_d}^d = \prod_{j=1}^d v_{i_j}^j
     .\] 
\end{definition}
\begin{definition}[Tensore di rango uno]
  Un tensore $ T \in \Rmsiz! $ ha \emph{rango uno} se si può scrivere come nell'Equazione
  \ref{eq:rk1}.
\end{definition} 
Questo ci permette di definire la seguente quantità fondamentale:
\begin{definition}[Rango di un tensore]
  Il rango di un tensore $ T \in \Rmsiz!$ è il minimo intero positivo $ r $ tale che $ T $ si può scrivere
  come somma di tensori di rango uno, e scriviamo $ \rk(T) = r $.
\end{definition}
...
quindi riadattare questa decomposizione per memorizzare $ T $ con costo $ O((n_1+\ldots+n_d)r)
...
\begin{remark}[Versione originale]
 ...
 definita in coordinate $ \Rmsiz $ tramite delle matrici $ U_1, \ldots, U_d $ tali che $ U_t = \left[ u_1^t \mid
...
\begin{remark}
Il rango CP di $ T $ coincide per definizione al rango di $ T $.
\end{remark}
...
\begin{definition}[Mode$-k$ unfolding]
  ...
  \[
  T(i,j) = T(\siwc{i}) \text{, con } j = \L(\siwc)
  .\] 
\end{definition}
...
\begin{lemma}[Legame tra il prodotto esterno e il prodotto di Kroneker]
  ...
  \item $ \Tm{T}{k} = v^k\left( v^d \krn \cdots \krn v^{k-1} \krn v^{k+1} \krn \cdots
    \krn v^1 \right)^{\top}$
  \item $\Tm{T}{\mathcal{R} \times \mathcal{C}} = (v^{r_\delta} \krn \cdots \krn
...
\[
  \Tn{y} = T \ttm U \in \R^{\ssiz{r}} \text{ con } \Tm{Y}{k} = U \Tm{T}{k}
.\] 
In componenti, $ \Tn{y}(\siwc{\alpha}) = \sum_{i_k = 1}^d T(\miwc) U_{\alpha, i_k} $.
\end{definition}
...
\begin{proposition}
  Dato un tensore $ T \in \Rmsiz* $ e delle matrici $ U \in \R^{m_1 \times n_1} $, $ V \in
  \R^{m_1 \times n_2} $ e $ W \in \R^{m_3 \times n_3} $. Sono fatti equivalenti:
  \begin{enumerate}[label=(\roman*)]
    \item $\Tn{y} = T \ttm[1] U \ttm[2] V \ttm[3] W \in \Rmsiz*[m]$.
    \item $\Vec(\Tn{y}) = (W \krn V \krn U) \cdot \Vec(T)$.
    \item $\Tm{Y}{1} = U\Tm{T}{1}(W \krn V)^{\top}$.
    \item $ \Tm{Y}{2} = V\Tm{T}{2}(W \krn U)^{\top} $.
    \item $\Tm{Y}{3} = W\Tm{T}{3}(V \krn U)^{\top}$.
  \end{enumerate}
\end{proposition}
\begin{proof}
\begin{description}
        \item[$(1) \iff (2)$] Mostriamo che $\Vec(T \ttm[1] U) = (I \krn I \krn U)
          \Vec(T)$.
...
come \emph{rango tipico} e verrà discusso nella Sezione~\ref{sec:rango_tipico}.
...
Mentre per le matrici il rango generico è unico e coincide con il massimo rango possibile, per i tensori di ordine superiore a due — in particolare sul campo reale $\mathbb{R}$ — possono coesistere più ranghi tipici. Un rango si dice tipico se l'insieme dei tensori che lo possiedono ha misura di Lebesgue strettamente positiva (ovvero se si presenta con probabilità non nulla quando si generano le entrate del tensore secondo una distribuzione di probabilità continua).

Sul campo complesso $\mathbb{C}$, il rango tipico (o generico) è sempre unico. Ad esempio, per tensori di formato $2 \times 2 \times 2$, il rango generico complesso è $2$. Tuttavia, sul campo reale $\mathbb{R}$, un tensore di formato $2 \times 2 \times 2$ può avere rango reale tipico pari a $2$ o $3$.

Per studiare empiricamente questo comportamento, abbiamo implementato un esperimento numerico in Python. Per ciascuno dei quattro formati analizzati ($2 \times 2 \times 2$, $3 \times 3 \times 2$, $3 \times 3 \times 3$ e $3 \times 3 \times 5$), abbiamo generato campioni di tensori casuali con entrate estratte indipendentemente sia da una distribuzione normale standard $\mathcal{N}(0, 1)$ sia da una distribuzione uniforme $\mathcal{U}(-1, 1)$. Il rango di ciascun tensore è stato stimato determinando il minimo valore di $r$ per cui l'algoritmo Alternating Least Squares (ALS) converge a un errore relativo inferiore a $10^{-5}$. Per evitare minimi locali, ogni esecuzione dell'algoritmo è stata ripetuta con 15 inizializzazioni casuali indipendenti.

La distribuzione empirica dei ranghi (riportata in Figura~\ref{fig:typical_ranks_dist}) mostra i seguenti risultati:
\begin{itemize}
  \item Per il formato $2 \times 2 \times 2$, circa il $70.5\%$--$74.0\%$ dei tensori presenta rango $2$, e il restante $26.0\%$--$29.5\%$ ha rango $3$.
  \item Per il formato $3 \times 3 \times 2$, circa il $29.5\%$--$39.5\%$ dei tensori presenta rango $3$, e il $60.5\%$--$70.5\%$ ha rango $4$.
  \item Per il formato $3 \times 3 \times 3$, il $100\%$ dei tensori ha rango $5$ per entrambe le distribuzioni, a conferma del fatto che $5$ è l'unico rango tipico reale per questa classe.
  \item Per il formato $3 \times 3 \times 5$, circa il $91.0\%$--$94.0\%$ dei tensori presenta rango $6$, un'ulteriore quota del $4.0\%$--$7.0\%$ ha rango $7$, e una frazione minore ($2.0\%$) converge a rango $5$.
\end{itemize}
Si nota come la distribuzione delle entrate (normale o uniforme) non alteri l'insieme dei ranghi tipici e produca frequenze empiriche del tutto analoghe, in perfetto accordo con il fatto che la proprietà di tipicità è di natura topologico-misurabile (ovvero legata agli insiemi di misura di Lebesgue positiva).
...
In ambito teorico, i ranghi tipici sul campo reale $\mathbb{R}$ sono stati caratterizzati per diverse famiglie di formati $m \times n \times p$ (assumendo senza perdita di generalità $2 \le m \le n \le p$). La Tabella~\ref{tab:typical_ranks} riassume i principali risultati noti in letteratura, tratti da Ballard e Kolda~\cite[p.~276, Tabella~16.2]{BK25}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    $2 \times n \times n$ & $\{n, n+1\}$ & ten Berge e Kiers (1999)~\cite{tenberge1999} \\
    $3 \times 3 \times 3$ & $5$ & ten Berge e Stegeman (2006)~\cite{tenberge2006} \\
    $3 \times 3 \times 5$ & $\{5, 6\}$ & ten Berge, Sidiropoulos e Rocci (2004)~\cite{tenberge2004} \\
    $3 \times 4 \times 8$ & $\{8, 9\}$ & Sumi et al. (2013)~\cite{sumi2013} \\
    $3 \times 5 \times 9$ & $\{9, 10\}$ & Choulakian (2010)~\cite{choulakian2010}; ten Berge (2011)~\cite{tenberge2011} \\
    $4 \times 4 \times 12$ & $\{12, 13\}$ & Friedland (2012)~\cite{friedland2012} \\
    $m \times n \times n(m-1)$ ($m > 2$, $n$ dispari) & $n(m-1)$ & ten Berge (2000)~\cite{tenberge2000b} \\
    $m \times n \times p$ ($n(m-1) < p < mn$) & $p$ & ten Berge e Kiers (1999)~\cite{tenberge1999}; ten Berge (2000)~\cite{tenberge2000b} \\
    $m \times n \times p$ ($p \ge mn$) & $mn$ & ten Berge e Kiers (1999)~\cite{tenberge1999}; ten Berge (2000)~\cite{tenberge2000b} \\
    \bottomrule
  \end{tabular}
\end{table}

\todo{change}
Come si evince dalla Tabella~\ref{tab:typical_ranks}, per i tensori di formato $2 \times n \times n$ i ranghi tipici reali sono sempre la coppia $\{n, n+1\}$~\cite{tenberge1999}, coerentemente con i nostri risultati empirici sul caso $2 \times 2 \times 2$. Analogamente, per il formato $3 \times 3 \times 3$ l'unico rango tipico reale è $5$~\cite{tenberge2006}, in perfetto accordo con il $100\%$ di convergenza ottenuto nella nostra simulazione. Per formati del tipo $m \times n \times n(m-1)$ con $m > 2$, quando $n$ è dispari il rango tipico è unico e pari a $n(m-1)$~\cite{tenberge2000b}; se $n$ è pari, il valore esatto del rango tipico non è ancora del tutto noto, ed è congetturato essere la coppia $\{p, p+1\}$~\cite{BK25}.
...
\begin{lemma}
  Dato un tensore $ T = \dsqr{U_1,\ldots,U_d} $, vale $ \Vec(T) = \left( U_d \krp U_{d-1}
