...
\begin{remark}
	Dato $ T \in A \otimes B \otimes C $, possiamo sempre passare da un tensore nello spazio $ A
		\otimes B \otimes C $ a un tensore in $ \K^{\bm{a} \times \bm{b} \times \bm{c}} $ così: fissate
...
\todo{aggiunto esempio, si può tenere? Non saprei però motivare meglio perché il tensore sia così}
\begin{example}[Tensore della matrix multiplication~\cite{landsberg2015geometry}]
	Nel Capitolo~\ref{chp:matmul} studieremo il tensore della moltiplicazione tra due matrici di taglia $ m \times n
	$ e $ n \times p $ come mappa
	bilineare $ \K^{m \times n} \times \K^{n \times p} \to \K^{m \times p} $ che può essere vista come
...
\begin{definition}[Canonical Polyadic Decomposition (CP)]
	...
	rango $ r \in \N $ se si può scrivere come somma di $ r $ tensori di rango uno
	\begin{equation*}
...
usando l'algoritmo ALS (che introdurremo in \S~\ref{alg:ALS}) su un campione di $ 200 $ tensori, facendo $ 120 $ restarts e un
...
\begin{proof}
	...
	sottomatrice di $ U_d \krn \cdots \krn U_2 $.\todo{Aggiunta questa giustificazoine} Notiamo che i fattori $ U_{j} $ hanno rango $ r_{j}
	$ in quanto
	\begin{equation*}
		\begin{aligned}
...
\begin{equation*}
	x^{\top}Zx = x^{\top}(C \krp B)^{\top}(C \krp B)x = ((C \krp B)x)^{\top}((C \krp B)x) = \|^2 (C \krp
	B)x \|_{2} \ge 0
\end{equation*}
