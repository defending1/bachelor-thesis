...
\begin{proposition}
	...
	\begin{enumerate}[label=(\roman*)]
		\item $Y = T \ttm[1] U \ttm[2] V \ttm[3] W \in \Rmsiz*[m]$.
		\item $\Vec(Y) = (W \krn V \krn U) \cdot \Vec(T)$.
...
Per studiare empiricamente questo comportamento, abbiamo implementato un esperimento numerico.
...
usando l'algoritmo ALS interrompendo una volta raggiunto un errore relativo inferiore a
$10^{-5}$. Per evitare minimi locali, ogni esecuzione dell'algoritmo è stata
ripetuta con alcuni riavvii.
La distribuzione empirica dei ranghi riportata in Figura~\ref{fig:typical_ranks_dist} mostra che la
distribuzione delle entrate (normale o uniforme) non alteri l'insieme dei ranghi tipici e produca
frequenze empiriche analoghe.\todo{non chiaro perché $ 3\times 3\times 5 $ produca anche tensori di
	rango $ 7 $}
