...
\textbf{Il problema inverso del rango:} Dato un intero positivo $ r $, nel caso matriciale è un problema semplice costruire una matrice di
rango $ r $. Invece, trovare un tensore di un certo rango è come trovare un ago in un pagliaio; ad esempio, nel
Capitolo~\ref{chp:matmul}\ldots \todo{discutere con R}
...
\begin{equation*}
   \Tn{x} = \sum_{j=1}^r u_j^1 \topp \cdots \topp u_j^d.
\end{equation*}
...
\begin{remark}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item $ \Tn{x} = \sum_{j=1}^r u_j^1 \topp \cdots \topp u_j^d$.
    \item $ \Vec(\Tn{x}) = \sum_{j=1}^r u_j^d \krn \cdots \krn u_j^1$.
    \item $\Tn{x}(\miwc) = \sum_{j=1}^r U_1(i_1,j)U_2(i_2,j)\cdots U_d(i_d,j)$.
  \end{enumerate}
...
impossibile~\cite{desilva2008tensorrankillposednessbest}.
