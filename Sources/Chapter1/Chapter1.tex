...
\begin{definition}[Reverse ordering]
  \emph{L'ordinamento inverso} su $ \mdom $ è la mappa biunivoca
\begin{equation*}
  \begin{aligned}
    \L^* \colon \mdom! &\to [\prod_{j=1}^d n_j]\\
    (\miwc) &\mapsto 1 + \sum_{j=1}^d s_j^*(i_j - 1),
  \end{aligned}
\end{equation*} dove $ s_j^* = \prod_{h=n}^{j+1} n_h $. La sua inversa è la mappa
\begin{equation*}
  \begin{aligned}
    (\L^*)^{-1} \colon [\prod_{j=1}^d n_j] &\to \mdom!\\
    l &\mapsto \left(1 + \left\lfloor \frac{ (l-1)\mod(n_1s_1^*)}{s_1^*}\right\rfloor, \ldots, 1 +
...
\begin{remark}
  La definizione differisce dall'ordinamento naturale solamente dalla definizione dei passi $ s_j $.
\end{remark}
