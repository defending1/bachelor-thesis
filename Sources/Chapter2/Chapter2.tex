...
costo $ O(n^2) $ per il calcolo di addizioni tra matrici.
Si congettura che $ \omega = 2 $, cioè che il costo della moltiplicazioni possa essere
asintoticamente pari a quello
delle moltiplicazioni. Dopo Strassen, nel 1978
...
\begin{equation}
  \label{eq:b_k_bil}
  b_k(x,y) = x^{\top}\Tn{x}(:,:,k)y = \sum_{i=1}^m\sum_{j=1}^n \Tn{x}(i,j,k)x_iy_j, \text{ per ogni
  } k = 1, \ldots, p
.\end{equation}
Da cui, 
\begin{equation}
  \label{eq:b_tuv}
 b(x,y) = \begin{bmatrix} b_1(x,y) \\ \vdots \\ b_p(x,y) \end{bmatrix} = \Tn{x} \ttm[1] x^{\top}
 \ttm[2] y^{\top}.
\end{equation}
\begin{remark}
...
\end{remark}
Data una decomposizione $ \Tn{x} = \dsqr{U,V,W} $ di rango $ r $, dalla Definizione~\ref{def:CP} abbiamo che $ \Tn{x} = \sum_{l=1}^r u_l \topp v_l \topp w_l $, o in componenti, $ \Tn{x}(i,j,k) = \sum_{l=1}^r u_{il}v_{jl}w_{kl} $. Inserendo questa relazione nella Equazione~\ref{eq:b_k_bil} troviamo,
