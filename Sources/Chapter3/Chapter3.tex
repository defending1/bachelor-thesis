...
\todo{la definizione di $ L $ è molto semplificata dal libro "Algebraic Complexity theory"~\cite{burgisser2013algebraic}}
Indichiamo con $ L $ la funzione che calcola la complessità aritmetica per il calcolo di una
funzione $ \varphi$. Fissato un campo $ \K \in \{\R,\C\}  $, denotiamo con
\begin{equation*}
  M(n) \defeq L \left( \left\{ \sum_{j=1}^n X_{ij}Y_{jl} \mid i,l \in \{1, \ldots, n\} \right\}  \right)
\end{equation*}
il numero totale di moltiplicazioni aritmetiche sufficienti a calcolare la moltiplicazione di due
matrici $ n\times n $. Determinare una formula per $ M(n) $ è lontano dagli strumenti attualmente
disponibili. Invece, il problema di approssimare la crescita asintotica della funzione $ n \mapsto
M(n) $ è fattibile; definiamo

\begin{definition}
  L'\emph{esponente $ \omega $ di una moltiplicazione matriciale} è il numero
\begin{equation*}
  \begin{aligned}
    \omega \defeq \inf\{\tau \in \R \mid &\text{ la moltiplicazione tra due matrici }
    n\times n  \\ & \text{ ha costo } O(n^\tau) \text{ operazioni aritmetiche} \}.
  \end{aligned}
\end{equation*}
In altre parole, $ \omega = \inf \{\tau \in \R \mid M(n) = O(h^\tau)\}  $.
\end{definition}

\begin{remark}
  Sia $ M(h) $ che $ \omega $ dipendono dal campo $ \K $. Siccome ci occuperemo di campi di
  caratteristica zero, la dipendenza dal campo non sarà rilevante per ai nostri fini.
\end{remark}

Osserviamo che $ M(n) \le n^2(2n - 1) $, e dal costo per il calcolo delle addizioni segue la stima$ M(n) \ge n^2 $, da
...
Nel calcolo della complessità, le decomposizioni low-rank possono ridurre notevolmente la il costo
di calcolo di valutazioni di funzioni. Se ad esempio $ M \in \R^{m\times n} $ è una matrice di rango uno, possiamo scrivere $ M =
u \topp v = uv^{\top} $ per certi $ u \in \R^m $ e $ v \in \R^n $; valutare $ Mx =
(uv^{\top})x = u(v^{\top}x) $ ha costo $ O(1) $. In generale da \todo{teorema fondamentale algebra
lineare} se $ M $ ha rango $ R $, possiamo scriverla come somma di $ R $ matrici di rango uno ed
ottenere un costo $ O(R) $ per $ Mx $. Quest'idea si generalizza in modo simile anche per forme
bilineari, come vedremo ora.
...
Riassumendo, $ \rk(\angb{n,n,n}) $ misura il numero di moltiplicazioni necessarie per
moltiplicare due matrici $ n\times n $ e dà una misura della complessità dell'algoritmo. Alla luce di ciò, possiamo riscrivere $ \omega $ come
\begin{equation}
...
\rk(\angb{n,n,n}) $~\cite{blaser2001}. Si veda~\cite{landsberg2011tensors}, Capitolo 11 per ulteriori dettagli.
...
ottimale, Bini et. Al BLR80 considerano il seguente algoritmo di moltiplicazione ridotto ottenuto
...
numericamente, minimizzando la norma di $ \angb{2,2,2}^{\text{red}} $ meno un tensore di rango
cinque che variava (metodo ALS??). Il loro computer continuava a produrre tensori di rango cinque
