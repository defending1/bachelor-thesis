...
\begin{definition}[Straight line program]
  Uno \emph{straight line program} (o \emph{circuito algebrico}) $ \Gamma $ su $ \K $ è una sequenza
...
\end{definition}
\begin{remark}
  Se i risultati intermedi sono dei vettori, l'output delle operazioni sopra appartiene appartiene a
  uno spazio vettoriale.
\end{remark}

\begin{definition}[Funzione costo]
  Sia $ \varphi : U \times V \to W $ una mappa bilineare tra $ \K $-spazi vettoriali di dimensione
  finita. Definiamo la \emph{funzione costo} associata a uno straight line program $ \Gamma $ come
  \begin{equation*}
    \begin{aligned}
...
\begin{definition}[Funzione di complessità aritmetica]
 ...
 \begin{equation*}
   \begin{aligned}
   L^{\text{tot}}(\varphi) \defeq \inf_{\Gamma \text{ straight line program}} \{ C_\Gamma^{\text{tot}}(\varphi)\} && \text{ la
   \emph{complessità totale di }\varphi, e} \\
   L(\varphi) \defeq \inf_{\Gamma \text{ straight line program}} \{ C_\Gamma(\varphi)\} && \text{ la
...
\end{proposition}
\begin{proof}
Data una decomposizione $ \Tn{t} = \dsqr{U,V,W} $ di rango $ r $, dalla Definizione~\ref{def:CP}
abbiamo che $ \Tn{t} = \sum_{l=1}^r u_l \topp v_l \topp w_l $, o in componenti, $ \Tn{t}_{i,j,k} =
...
\end{proof}
Quindi valutare $ \varphi $ richiede esattamente $ r = \rk(\Tn{t}) $ active multiplications (e $
...
\begin{equation}
  \label{eq:mulvec}
\Vec(C^\top) =\varphi(x,y) = \Tn{T} \ttm[1] \Vec(A)^{\top} \ttm[2] \Vec(B)^{\top}
,\end{equation}
...
allargando la matrice. Possiamo racchiudere questa proprietà nell seguente:
...
\begin{remark}
  ...
  tensore $ \algob{m,n,p} $ e scrivendo, attraverso una serie di permutazioni (che non specifichiamo), la fattorizzazione di $ \algob{mnp} $ di rango $ r^3 $, che produce un algoritmo di complessità $ O(n^{\log_{abc}r^3}) =
  O(n^{3\log_{abc}r})$
\end{remark}
