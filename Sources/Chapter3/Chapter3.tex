...
\begin{definition}[Straight line program]
  Uno \emph{straight line program} (o \emph{circuito algebrico}) su un campo $ \K $ è una procedura algoritmica sequenziale che consiste solamente di istruzioni
  della forma
  \begin{equation*}
...
Dal ragionamento sopra, $ \rk(\angb{n,n,n}) $ è dato dal numero di active multiplication nella molitiplicazione
...
\begin{equation}
  ...
  \omega = \lowlim_{n \to +\infty} \log_n \left( \rk(\angb{n,n,n}) \right).
\end{equation}
\todo{FIX errore nella formula}
Ad esempio, l'algoritmo di Strassen~\cite{strassen69} mostra che $ \rk(\angb{2,2,2}) \le 7$, e
come dimostrato da Winograd~\cite{winograd1971}, $ \rk(\angb{2,2,2}) = 7$, ovvero non è possibile moltiplicare due
matrici $ 2\times 2 $ con meno di sette moltiplicazioni.
Nei casi diversi da $ \angb{2,2,2} $ il problema si complica, e attualmente abbiamo alcune stime più lasche: già
per matrici $ 3\times 3 $, sappiamo solo che $ 19 \le \rk(\angb{3,3,3}) \le 23 $~\cite{BLASER200343,
bams/1183537626}, mentre la migliore stima astintotica dal basso per il caso di matrici $ n\times n $ è $ \frac{5}{2}n^2 - 3n \le
\rk(\angb{n,n,n}) $~\cite{blaser2001}. Si veda il capitolo 11 di~\cite{landsberg2011tensors} per
...
\rk(\angb{n,n,n}) \le r $, allora $ \rk(\angb{n^i,n^i,n^i}) \le
...
\begin{equation*}
  \rk(\angb{n^i,n^i,n^i}) = \rk(\angb{n,n,n}^{\otimes i}) \le \rk(\angb{n,n,n})^i \le r^i.
\end{equation*}
...
Se invece $ n $ è un generico intero positivo, dato un algoritmo con un caso base fissato $
\angb{n,n,n} $, è possibile ottenerne uno per
$ \angb{\hat{n}, \hat{n}, \hat{n}} $, con $ \hat{n} >  n $, allargando la matrice aggiungendo zeri
fino alla dimensione
$ n^{\left\lceil \log \hat{n} \right\rceil } $. In particolare, si ha che
\begin{equation*}
  \begin{aligned}
    \rk(\angb{n,n,n}) \le \rk(\angb{n^{\left\lceil \log \hat{n} \right\rceil }})
  \end{aligned}
\end{equation*}
...
Il rango è monotono crescente, ovvero dati due interi positivi $ n \le n' $, allora $
\rk(n,n,n) \le \rk(n', n', n') $.
\end{proposition}
...
\begin{proof}
  ...
  \begin{equation*}
    r \defeq \rk(\angb{m,m,m}) \le m^{\theta + \varepsilon}.
  \end{equation*}
...
\begin{remark}
  ...
  tensore $ \angb{m,n,p} $ e scrivendo, attraverso una serie di permutazioni, la fattorizzazione di $ \angb{mnp, mnp,
  mnp} $ di rango $ r^3 $, che produce un algoritmo di complessità $ O(n^{\log_{abc}r^3}) =
  O(n^{3\log_{abc}r})$
\end{remark}
...
\end{aligned}
\]
Usando ?? possiamo scrivere l'algoritmo $ \angb{2,2,2} $ in forma tensoriale in formato CP:
\todo{Si vede come si ricava ma non so spiegare come si ottiene}
\begin{equation}
  \label{eq:tensor_222}
  \begin{aligned}
    \angb{2,2,2} &= a_{11} \topp b_{11}\topp c_{11} + a_{12}\topp b_{21} \topp c_{11} \\
...
stessa struttura a blocchi $ \angb{2,2,2} $.
...
ponendo a zero l'entrata $ a_{22} $ di $ \angb{2,2,2} $:
\begin{equation*}
  \begin{bmatrix}
    a_{11} & a_{12} \\
    a_{21} & 0
...
Che è somma di sei tensori elementari distinti, quindi $ \rk{\angb{2,2,2}^{\text{red}}} \le 6 $.
Bini et. al. provarono a cercare un'espressione di rango cinque per $ \angb{2,2,2}^{\text{red}} $
numericamente, calcolando
\begin{equation*}
  \min_{\substack{T \in \R^{n\times n\times n} \\ \rk(T) = 5}}{\| \angb{2,2,2}^{\text{red}} - T\| }
\end{equation*}
 norma di $ \angb{2,2,2}^{\text{red}} $. Il loro computer continuava a produrre tensori $ T $ di rango cinque
