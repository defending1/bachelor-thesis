...
impiegano un asintoticamente numero minore di operazioni aritmetiche rispetto all'algoritmo
...
Date due matrici $ n^i \times n^i $, possiamo vedere la loro moltiplicazione come il prodotto
...
applicando l'Equazione~\ref{eq:omega_as_rank}, segue che $ \omega \le \log_{n_0} r $. Ad esempio, per
l'algoritmo di Strassen (che introdurremo a breve) avremo che $ r = 7, n_0 = 2 $, da cui $ \omega \le \log_{2} 7 < 2.81 $. Abbiamo
...
La prossima proposizione risponde al problema inverso problema inverso: dato un intero positivo $ r $, è possibile
trovare un algoritmo con $ r $ moltiplicazioni?
...
\end{equation*}
a noi resta solo da assegnare i termini $ w_{i} $. Una regola per farlo è questa: guardiamo quali
termini $ m_{r} $ formano le entrate $ c_{ij} $, e impostiamo $ w_{r} $ uguale al valore $ c_{ij} $
dove si trova il rispettivo $ m_{r} $ (se fossero presenti più copie di un certo $ m_{r} $, i $
	w_{r} $ sarebbero combinazione lineare dei termini $ c_{ij} $). Nel caso di $ c_{11} $ ad esempio, sono presenti $
	m_1 $ e $ m_2 $, quindi $ w_1 =w_2 = c_{11} $. Procedendo in questo modo per gli altri termini troviamo,
\begin{equation*}
	\begin{aligned}
		w_1 & = w_2 = c_{11}, \qquad w_5 = w_6 = c_{12}, \\
		w_3 & = w_4 = c_{21}, \qquad w_7 = w_8 = c_{22}.
	\end{aligned}
\end{equation*}
...
ragionamento dell'Equazione~\todo{mst} del Teorema di Strassen, troviamo che $ M(n) = O(n^{\log_{2}
			8}) = O(n^3)$. Aggiungiamo che, in letteratura a volte si usa un risultato chiamato Master
Theorem~\cite{cormen2022introduction} per dare
queste stime asintotiche.
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
Dall'equazione sopra possiamo scrivere i termini $ w_{r} $ come combinazione lineare dei $ c_{ij} $
dati raccogliendo i termini $ m_{r} $. In altre parole,
\begin{equation*}
	\begin{aligned}
		w_1 & = c_{11} + c_{22}  \\
		w_2 & = c_{21} - c_{22}  \\
...
\todo{non chiaro passaggio con forma trilineare, cioè ottengo la bilinear computation con $ tr(ABC^{\top}) $}
