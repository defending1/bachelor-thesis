...
dalla notazione della Definizione~\ref{def:LtotL}, poniamo $ \varphi = \angb{n} $ e scriviamo le sue coordinate $
\varphi_1, \ldots, \varphi_{n^2} $ come nell'Equazione~\ref{eq:b_active}.
Fissato un campo $ \K \in \{\R,\C\}  $, e delle matrici $ A, B \in \R^{n\times
n}$, denotiamo con
\begin{equation}
  \label{eq:def_Mn}
  M_{\K}(n) \defeq L^{\text{tot}} \left( \angb{n} \right) =  L^{\text{tot}} \left( \left\{
...
trovare un algoritmo con $ r $ moltiplicazioni? La risposta precisa è data dal seguente risulato.
...
\end{definition}
\begin{example}
  La mappa bilineare $ \angb{n} $ è concise.
\end{example}
...
\begin{proof}[Dimostrazione del Teorema di Strassen]
  ...
  \begin{equation}
    \label{eq:r_estimate}
    r \defeq \rk(\angb{n}) \le n^{\theta + \varepsilon}.
  \end{equation}
...
L'Equazione sopra rappresenta l'algoritmo di moltiplicazione a blocchi tra $ A
  ...
  Siccome $ \angb{n} $ è concise, abbiamo che $ r \ge n^2 $. Fissando $ n $ (e quindi $ r $),
