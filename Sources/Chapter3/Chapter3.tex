...
\begin{definition}[Circuito algebrico]
  Un \emph{circuito algebrico} è un modello computazionale sequenziale dove ogni gate logico rappresenta una
  delle seguenti operazioni algebriche su un campo $ \K $ fissato: addizione, sottrazione,
  moltiplicazione, divisione di due termini, e moltiplicazione per scalare.
\end{definition}
...
\begin{equation*}
  \begin{aligned}
    \omega_{\K} \defeq \inf_n\{\tau \in \R \mid &\text{ la moltiplicazione tra due matrici }
    n\times n  \\ & \text{ ha costo } O(n^\tau) \text{ operazioni aritmetiche} \}.
  \end{aligned}
\end{equation*}
...
\begin{remark}
Dalla notazione $ M_{\K}(h) $ e $ \omega_{\K} $ è dovuta alla dipendenza dal campo $ \K $. Di
seguito scriveremo solo $ \omega $, il quanto il seguente risultato avanzato afferma che l'esponente
dipende solo dalla caratteristica del campo.
\end{remark}
...
Osserviamo che $ M_{\K}(n) \le n^2(2n - 1) $. Analizzando la scrittura del prodotto tra matrici
nell'Equazione~\ref{eq:def_Mn},  $ M_{\K}(n) \ge n^2 $\todo{COME?}, da
...
più scesa sotto $ \omega < 2.373 $ \cite{wil11,gall14,stothers2010complexity, alman2024}.
\begin{figure}[ht]
  \centering
...
Se $ n = h^i $, possiamo vedere la moltiplicazione di due matrici $ h^i \times h^i $
come la moltiplicazione di matrici $ h\times h $ sull'anello delle matrici $ h^{i-1}\times
h^{i-1} $ suddividendo in blocchi le matrici di partenza. Questo dice anche che, se $
\rk(\angb{h,h,h}) \le r $, allora $ \rk(\angb{n^i,n^i,n^i}) \le
  r^i$.\todo{si può dimostrare con un poco di conti algebrici, lo facciamo?}
Dato un algoritmo per un caso base fissato $ \angb{n,n,n} $, è possibile ottenerne uno per
$ \angb{\hat{n}, \hat{n}, \hat{n}} $, con $ \hat{n} >  n $ suddividendo la matrici in
blocchi, o riempendole di zeri fino a far combaciare le dimensioni con quelle del caso base. In
particolare, 
\end{remark}
