...
Da un punto di vista pratico, non è detto che le stime ottimali si traducano in algoritmi più veloci
o stabili di quello classico, ad esempio l'algoritmo di Strassen, che presenteremo, risulta ottimale
in molte situazioni~\cite{dalberto2023strassensmatrixmultiplicationalgorithm}.
\begin{figure}[ht]
	\centering
...
\begin{proof}[Dimostrazione del Teorema di Strassen]
	...
	\begin{equation}
		...
		M(n^i) = O(r^i).
	\end{equation}
	Dalla proposizione~\ref{prop:rk_monotony} segue che $M$ è una successione monotona, da cui per
	ogni $ h = n^i \in \N \setminus \{0\}  $\todo{non sono certo che $ h = n^i $, io l'ho interpretato
		così},
	\begin{equation*}
...
\begin{remark}
	Riassumendo, $ \rk(\algob{n}) $ è dato dal numero di active multiplication nella molitiplicazione
...
Il suo costo può essere calcolato con il Teorema~\ref{th:master}, ed è $ O(n^3) $.
