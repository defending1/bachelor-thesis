...
def visualize_eem_cp(mat_path: Path = Path("EEM18.mat"),
    ...
    """
    Fit rank-3 NN-CP model with ALS stopping criterion tol=1e-4 (relative error change < 10^-4),
    sort components by weight, align factors with true mixtures,
    and output a publication-quality 3x3 plot of loadings/spectra.
    Compound names ordered as: Phe, Trp-Gly, Val-Tyr-Val.
    """
    setup_plot_style()
    ...
    # 1. Fit CP with ALS stopping threshold tol=1e-4 (change in relative error < 10^-4)
    ...
    # Scale sample factors and true mixtures to concentration scale * 1e6 (0 to 6 * 10^6)
    ...
    # Compound names per component index (Phe, Trp-Gly, Val-Tyr-Val)
    compound_names = ["Phe", "Trp-Gly", "Val-Tyr-Val"]

    # Get wavelengths
    em_range, ex_range = get_wavelength_ranges(mode_ranges)

    # Create layout with tighter column spacing
    ...
    color_watermark = '#FDB87D' # Light orange for background watermark

    for j in range(3):
        # Background watermark centered in the middle (Emission) subplot of each row (50pt light orange, 50% opacity, unclipped)
        ...
        # Y-axis limits 0 to 6.5 * 10^6 with ticks at 0, 2, 4, 6
        ax_sample.set_ylim([0, 7.0e6])
        ax_sample.set_yticks([0, 2e6, 4e6, 6e6])
        ax_sample.set_yticklabels(['0', '2', '4', '6'], fontsize=9)
        
        # Add scientific multiplier \cdot 10^6 at top left of each left subplot
    ...
    # Explicit subplots adjustment for tighter horizontal column spacing (wspace=0.20)
