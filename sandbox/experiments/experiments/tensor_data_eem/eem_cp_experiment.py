...
from experiments.tensor_data_eem.decomposition import run_experiment
...
def plot_results(results: dict, output_dir: Path, sandbox_dir: Path = None):
    ...
    color_err = "#d62728"
    ax1.set_xlabel(r"Rango ($r$)", fontsize=11.5)
    ax1.set_ylabel(
        r"Errore relativo ($\|T - \hat{T}\| / \|T\|$, \%)",
        color="black",
        fontsize=11.5,
    )
    line1 = ax1.plot(
    ...
    line_target = ax1.axhline(
        ...
        linestyle="--",
        linewidth=1.5,
        label=f"Tolleranza target $\\epsilon$ ({target_err * 100.0:.1f}\\%)",
    )
    line_final = ax1.axvline(
        ...
        linestyle=":",
        linewidth=1.5,
        label=f"Rango finale ($r={final_rank}$)",
    )
    ax1.set_xticks(ranks)
    ax1.tick_params(axis="both", labelsize=11.5, labelcolor="black")
    ax1.grid(True, linestyle=":", alpha=0.6)
    ax1.set_ylim([0, max(errors_pct) + 8])

    lines = line1 + [line_target, line_final]
    labels = [l.get_label() for l in lines]
    ax1.legend(lines, labels, loc="upper right", framealpha=0.9, fontsize=11.5)
    ...
    weights, factors = final_cp.weights, final_cp.factors
    ...
    axes[0].set_xlabel("Indice del campione (1..18)")
    ...
    axes[1].set_xlabel("Lunghezza d'onda (nm)")
    axes[1].set_ylabel("Intensità (u.a.)")
    axes[1].grid(True, linestyle=":", alpha=0.5)
    axes[1].legend()

    for r in range(final_rank):
        axes[2].plot(ex_range, factors[2][:, r], linewidth=2, label=f"Comp. {r + 1}")
    axes[2].set_xlabel("Lunghezza d'onda (nm)")
    axes[2].set_ylabel("Intensità (u.a.)")
...
def main():
    ...
    parser.add_argument(
        "--plot-dir",
        type=Path,
        default=script_dir.parents[3] / "Sources" / "Chapter4" / "figures",
