...
% Enable column format arguments for amsmath matrices
\makeatletter
\renewcommand*\env@matrix[1][*\c@MaxMatrixCols c]{%
  \hskip -\arraycolsep
  \let\@ifnextchar\new@ifnextchar
  \array{#1}\rule[-1.2ex]{0pt}{3.5ex}}
\makeatother
...
\NewEnviron{warning}{%
  \par\vspace{1ex}       % Small space before the environment
  \noindent\uline{\textit{Avvertenza}}: \BODY
  \par\vspace{1ex}       % Small space after the environment
}
...
\newglossaryentry{indexset}{%
  name        = {$[n]$},
  description = {Insieme dei numeri $\{1, 2, \ldots, n\}$},
  sort        = {n},
}
...
\newglossaryentry{annihilator}{%
  name        = {$U^\perp$},
  description = {Annullatore del sottospazio $U$},
  sort        = {Uperp},
}
...
\newglossaryentry{rank}{%
  ...
  description = {Rango tensoriale e rango matriciale},
  sort        = {rk},
}
...
\newglossaryentry{krank}{%
  name        = {$\mathbf{k}_{\mathcal{S}}$},
  description = {Kruskal rank ($k$-rango) dell'insieme $\mathcal{S}$},
  sort        = {krk},
}
...
\begin{flushright}
	\textit{A nonna Pier,} \\
	\textit{per i gustosissimi ``pranzetti''.}
\end{flushright}
\vspace*{\stretch{2}}
\clearpage
